...
3. [ ] **Prove the live path with a minimal UI** *(reserved for v0)*
   - [ ] Connect a deliberately plain textarea and result view to `/api/intake`.
   - [ ] Verify a complete input-to-structured-output flow before investing in visual implementation.

4. [ ] **Create the interface with v0** *(reserved for v0)*
   - [ ] Generate a calm, accessible insurer-style intake interface against the proven `/api/intake` contract: narrative textarea, example claims, submit action, result panel, loading state, validation state, and API-error state.
   - [ ] Bring the selected v0-generated components and styling into this repository. This repository remains the source of truth.
   - [ ] Replace the temporary UI while preserving the API contract and its validation behavior.
